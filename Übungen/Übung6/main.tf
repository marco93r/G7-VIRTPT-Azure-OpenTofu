...
resource "azurerm_public_ip" "pip_vms" {
  count               = var.vm_count
  name                = "pip-vm-ubuntu-${count.index}"
  resource_group_name = azurerm_resource_group.rg_main.name
  location            = var.location
...
resource "azurerm_network_interface" "nic_vms" {
  count               = var.vm_count
  name                = "nic-vm-ubuntu-${count.index}"
  location            = var.location
  resource_group_name = azurerm_resource_group.rg_main.name
  ...
  ip_configuration {
    ...
    public_ip_address_id          = azurerm_public_ip.pip_vms[count.index].id
  }
}
...
resource "azurerm_linux_virtual_machine" "vms" {
  ...
  network_interface_ids = [
    azurerm_network_interface.nic_vms[count.index].id,
  ]
...
output "pip_ip" {
  value = azurerm_public_ip.pip_vms
}